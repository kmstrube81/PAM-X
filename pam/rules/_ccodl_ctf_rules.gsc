...
{
    // Hostname and MOTD 
	setcvar("sv_hostname" , "CCoDL Match in Progress"); 
	setcvar("scr_motd" , "ccodl.burstfire.net"); 

	// Logo
	game["leaguestring"] = &"CCoDL CTF Match Mode";  //NOTE!!! NEVER REMOVE THE & SYMBOL OR SERVER WILL CRASH
    ...
    maps\mp\uox\_uox_vars::updateCvar("scr","ot_scorelimit", "1", "ctf");
	...
	setcvar("g_timeoutLength", "120"); //The length of each timeout. 
	setcvar("g_timeoutRecovery", "10"); //The length of the preparation period which occurs after a time-in is called, or after a timeout expires.  This recovery period is used to alert all players that play is about to begin. 
	setcvar("g_timeoutBank", "120"); //The total amount of time a team can spend in timeout.
